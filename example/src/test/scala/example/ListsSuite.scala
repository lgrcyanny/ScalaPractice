...
class ListsSuite extends FunSuite {
  ...
  test("sum of empty lists ") {
    assert(sum(List()) === 0)
  }

  test("sum of negative values") {
    assert(sum(List(-10, -11, -12, -13)) === -46)
  }
  ...
  test("max of negative values") {
    assert(max(List(-10, -20, -30)) == -10)
  }

  test("max with repeated values") {
    assert(max(List(1, 2, 2, 2, 3, 3, 4, 6, 6, 8, 8)) == 8)
  }
}
